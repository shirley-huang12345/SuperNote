...
public class ColorPickerViewCustom extends View {
	...
	public ColorPickerViewCustom(Context c, AttributeSet attrSet) {
		...
		mSharedPreference = c.getSharedPreferences(MetaData.PREFERENCE_NAME, Activity.MODE_PRIVATE);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		setMeasuredDimension((mColorPickerWidth * MetaData.DPI / 160), (mColorPickerHeight * MetaData.DPI / 160));
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (x > 0 && x < (mColorPickerWidth * MetaData.DPI / 160) && y > 0 && y < (mColorPickerHeight * MetaData.DPI / 160)) {
			mCurrentX = (int) x;
			mCurrentY = (int) y;
			int transX = mCurrentX * 360 / (mColorPickerWidth * MetaData.DPI / 160);
			int transY = mCurrentY * 100 / (mColorPickerHeight * MetaData.DPI / 160);
	...
	{
		Bitmap colorBitmap = Bitmap.createBitmap((mColorPickerWidth * MetaData.DPI / 160), 
        		(mColorPickerHeight * MetaData.DPI / 160), Config.ARGB_8888);
		Canvas canvas = new Canvas(colorBitmap);
		for (int x = 0; x < (mColorPickerWidth * MetaData.DPI / 160); x++) {
			int[] colors = new int[10];
			int j = x * 360 / (mColorPickerWidth * MetaData.DPI / 160) ;
			int y = 100 * j;
			for (int i = 0; i < 10; i++, y = y + 10)
				colors[i] = mMainColors[y];

			Shader shader = new LinearGradient(0, 0, 0, (mColorPickerHeight * MetaData.DPI / 160), colors, null,
					Shader.TileMode.CLAMP);
			mPaint.setShader(shader);
			canvas.drawLine(x, 0, x, (mColorPickerHeight * MetaData.DPI / 160), mPaint);
		}
